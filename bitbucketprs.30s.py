...
# https://github.com/mikeybeck/bitbar-bitbucketPRs
...
config = ConfigParser()
config.read(os.path.dirname(os.path.realpath(__file__)) + '/.bitbucket-config.ini')
preferences = config._sections

BITBUCKET_HOST = preferences['preferences']['bitbucket_host']
PRIVATE_TOKEN = preferences['preferences']['private_token']
USER_SLUG = preferences['preferences']['user_slug']
SORT_ON = preferences['preferences']['sort_on']
ABBREVIATION_CHARACTERS = int(preferences['preferences']['abbreviation_characters'])
...
pr_status = dict(
    NEEDS_WORK="iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAfElEQVQ4T2NkQAOpqanasxxmX0EXB/HTDqTqzJ49+yqyHCOMExoayrYqYPVPbBrRxcI2hLKvXr36F0gcbAApmmGGwQwBG/B/KcN/YmxGV8MYzcDIiM/PhAwFhQkjubbDDB81gIGB8jCgOBopTkhUScqkGoKRmZCTLKnZGQCX5FCXyXSbYwAAAABJRU5ErkJggg==",
...
def pr_should_be_reviewed_by_me(pr):
    ...
    if len(reviewers_filtered) > 0 and reviewers_filtered[0]['status'] != "APPROVED":
        pr['overallStatus'] = reviewers_filtered[0]['status']
        return pr


def pr_is_marked_as_needs_work(pr):
    reviewers_status = list(map(lambda reviewer: reviewer['status'], pr['reviewers']))

    if "NEEDS_WORK" in reviewers_status:
        pr['overallStatus'] = "NEEDS_WORK"
    elif "APPROVED" not in reviewers_status:
        pr['overallStatus'] = "UNAPPROVED"
    else:
        pr['overallStatus'] = "APPROVED"
...
def abbreviate_string(s):
    if len(s) > ABBREVIATION_CHARACTERS:
        return s[:ABBREVIATION_CHARACTERS] + "..."
    else:
        return s
...
def print_prs(pr_type, pull_requests):
    ...
    for slug, slug_prs in itertools.groupby(prs_sorted_by_slug, key=lambda pr: pr['slug']):
        slug_prs_list = list(slug_prs)
        print(slug + " (" + str(len(slug_prs_list)) + ")")

        prs_sorted_by_to_ref = sorted(slug_prs_list, key=lambda p: p['to_ref'])

        for to_ref, to_ref_prs in itertools.groupby(prs_sorted_by_to_ref, key=lambda pr: pr['to_ref']):
...
if __name__ == "__main__":
    ...
    if exception is not None:
        ...
        # Set menubar icon
        total_prs_with_work = str(len(prs_to_review) + len(prs_authored_with_work))
        print(total_prs_with_work + " | templateImage=" + pr_image)
